...
{
	$to = 'teethwhitening.in.ua@gmail.com';
	$subject = 'Заказ с сайта (ru основной, низ)';
	
	$message = htmlentities($subject,ENT_COMPAT,'UTF-8').  "\r\n";
	
	$message .= 'Новый заказ'.  "\r\n"; 
	$message .= 'Имя:' . htmlentities($_REQUEST["name1"],ENT_COMPAT,'UTF-8').  "\r\n";
	$message .= 'Телефон:' . htmlentities($_REQUEST["phone1"],ENT_COMPAT,'UTF-8').  "\r\n";
	$message .= 'Email:' . htmlentities($_REQUEST["email1"],ENT_COMPAT,'UTF-8').  "\r\n";
	$message .= 'Сообщение:' . htmlentities($_REQUEST["message1"],ENT_COMPAT,'UTF-8').  "\r\n";
	...
	$headers = 'От кого:   support@amazingwebstudio.com' . "\r\n" . '' . $formEmail .  "\r\n" .'' . phpversion() . "\r\n" . '' . "\r\n";
